...
// Target line - równomierny wzrost do 1M PLN (na koniec każdego miesiąca)
// Używamy {x, y} format dla linear x-axis, x: 1-12 odpowiada końcowi każdego miesiąca
const targetData = months.map((_, i) => ({ x: i + 1, y: Math.round((yearlyTarget / 12) * (i + 1)) }))

// Actual cumulative data na koniec każdego pełnego miesiąca
const monthlyRevenue = [287000, 387000, 483000, 594000, 683000, 793000, 94000, 83000, 85000, 85000, 83000, 72000]
const cumulativeRevenue: number[] = []
monthlyRevenue.reduce((acc, val, i) => {
  cumulativeRevenue[i] = acc + val
  return cumulativeRevenue[i]
}, 0)

// Aktualna wartość (proporcjonalna do dnia w miesiącu)
const previousMonthTotal = currentMonth > 0 ? cumulativeRevenue[currentMonth - 1] : 0
const currentMonthProgress = monthlyRevenue[currentMonth] * dayProgress
const currentRevenue = Math.round(previousMonthTotal + currentMonthProgress)
...
for (let i = 0; i < currentMonth; i++) {
  actualData.push({ x: i + 1, y: cumulativeRevenue[i] })
}
// Dodaj aktualny punkt na proporcjonalnej pozycji w bieżącym miesiącu
actualData.push({ x: currentPosition + 1, y: currentRevenue })
    ...
    tooltip: {
      ...
      callbacks: {
        ...
        label: (context: any) => {
          ...
          return `${context.dataset.label}: ${(value / 1000).toFixed(0)}k PLN`
        }
      }
    }
  },
...
const targetAtCurrentPosition = Math.round((yearlyTarget / 12) * (currentPosition + 1))
const percentOfTarget = Math.round((currentRevenue / targetAtCurrentPosition) * 100)
</script>
    ...
    <template #header>
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <h3 class="font-semibold text-gray-900 dark:text-white">Obrót</h3>
        </div>
        <div class="flex items-center gap-2">
    ...
    <template #footer>
      <div class="flex justify-between items-center">
        <div class="flex items-center gap-3">
          <div class="px-3 py-2 bg-green-100 dark:bg-green-900/30 rounded-lg">
            <p class="text-xs text-green-600 dark:text-green-400">{{ currentDate }}</p>
            <p class="text-lg font-bold text-green-700 dark:text-green-300">{{ (currentRevenue / 1000).toFixed(0) }}k PLN</p>
          </div>
        </div>
        <div class="text-right">
          <p class="text-xs text-gray-500 dark:text-gray-400">Cel roczny</p>
          <p class="text-lg font-semibold text-gray-900 dark:text-white">1 000k PLN</p>
        </div>
      </div>
    </template>
  </UCard>
